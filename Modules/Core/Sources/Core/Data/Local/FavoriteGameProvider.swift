...
//    }()
    ...
    public func getAllFavorites(completion: @escaping(_ games: [GameModel]) -> Void) {
    ...
    public func getFavoriteBySlug(_ slug: String, completion: @escaping(_ game: GameModel) -> Void) {
        let taskContext = newTaskContext()
        taskContext.perform {
            let fetchRequest = NSFetchRequest<NSManagedObject>(entityName: "Favorite")
    ...
    public func addFavorite(_ game: GameModel, completion: @escaping() -> Void) {
        let taskContext = newTaskContext()
...
//        }
    ...
    public func deleteFavorite(_ slug: String, completion: @escaping() -> Void) {
        let taskContext = newTaskContext()
        taskContext.perform {
            let fetchRequest = NSFetchRequest<NSFetchRequestResult>(entityName: "Favorite")
            fetchRequest.fetchLimit = 1
    ...
    public func getMaxId(completion: @escaping(_ maxId: Int) -> Void) {
        let taskContext = newTaskContext()
        taskContext.performAndWait {
            let fetchRequest = NSFetchRequest<NSManagedObject>(entityName: "Favorite")
